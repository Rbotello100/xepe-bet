-- =====================================================================
-- atomic_bets_v3: ampliar whitelist de pick + agregar whitelist de market_type
-- =====================================================================
--
-- Las RPCs anteriores validaban v_pick contra la lista 1X2 hardcodeada.
-- Tier 1+2 agrega 13 picks nuevos (double_chance, btts, dnb, totals).
-- Tambien sumamos validacion de market_type — antes era texto libre.
--
-- Las funciones DE LA APP son las MISMAS que en v2 (placeBet/placeParlay
-- Server Actions). El cambio es solo en el cuerpo de las RPCs SQL.
-- =====================================================================

-- 1) place_bet_atomic — agregar validacion de market_type + pick ampliado
...
BEGIN
  ...
  IF p_server_odds IS NULL OR p_server_odds < 1.01 OR p_server_odds > 1000 THEN
    ...
    RETURN;
  END IF;
  -- Market whitelist (defensa final paralela al CHECK constraint).
  IF p_market_type NOT IN ('1x2','double_chance','btts','draw_no_bet','totals_1.5','totals_2.5','totals_3.5') THEN
    RETURN QUERY SELECT false, NULL::uuid, NULL::numeric, NULL::numeric, 'invalid_market'::text;
    RETURN;
  END IF;
  -- Pick whitelist ampliada.
  IF p_pick NOT IN (
...
-- 2) place_parlay_atomic — ampliar whitelist de pick + agregar de market_type
...
BEGIN
  ...
  -- Validar cada leg DENTRO de la TX
  FOR v_leg IN SELECT * FROM jsonb_array_elements(p_legs) LOOP
    v_match_id := (v_leg->>'match_id')::uuid;
    v_pick := v_leg->>'pick';
    v_market := v_leg->>'market_type';

    -- Market whitelist (defensa final).
    IF v_market NOT IN ('1x2','double_chance','btts','draw_no_bet','totals_1.5','totals_2.5','totals_3.5') THEN
      RETURN QUERY SELECT false, NULL::uuid, NULL::numeric, NULL::numeric, 'invalid_market'::text;
      RETURN;
    END IF;
