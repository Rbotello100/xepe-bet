...
/**
 * "Mejor apuesta del dia" — card destacada en dorado con modal de detalle
 * al click. El modal muestra todo el contexto: match, mercado, pick, cuando
 * apostó, cuota, y quien mas se sumo (backers).
 */
  ...
  return (
    <>
      <button
        ...
        className="group relative w-full overflow-hidden rounded-lg border border-gold/40 bg-[linear-gradient(160deg,color-mix(in_oklab,var(--color-gold)_16%,var(--color-card)),var(--color-card)_55%)] px-[18px] pb-4 pt-[18px] text-left transition-colors hover:border-gold/70"
      >
        ...
        <div className="mt-3.5 flex items-center gap-3 border-t border-gold/20 pt-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="font-mono text-lg font-bold text-win">${bet.payout.toLocaleString('es-CL')}</span>
          </div>
          <div className="ml-auto rounded-full bg-sunken px-2.5 py-1 text-[11px] text-muted">
            {bet.backers} la {bet.backers === 1 ? 'sigue' : 'siguen'}
          </div>
        </div>
      ...
      {open && (
        ...
        >
          <div
            className="relative w-full max-w-md rounded-xl border border-gold/40 bg-card p-5 shadow-2xl"
          ...
          >
            ...
            {/* Apuesta */}
            <div className="mb-4 rounded-lg border border-gold/30 bg-gold/5 px-3 py-3">
              ...
              <div className="mt-1.5 flex items-center gap-2">
                <span className="rounded-full bg-gold/15 px-2.5 py-0.5 font-mono text-sm font-bold text-gold">
                  x{bet.odds.toFixed(2)}
                </span>
                <span className="text-xs text-muted">
                  ${bet.stake.toLocaleString('es-CL')} → <span className="font-bold text-win">${bet.payout.toLocaleString('es-CL')}</span>
                </span>
              </div>
            </div>
            ...
            <div className="mb-3">
              ...
              {bet.starts_at && (
                <p className="text-[11px] text-muted">
                  {formatDate(bet.starts_at)}
                  {bet.match_status === 'live' && <span className="ml-2 rounded-full bg-danger/15 px-1.5 py-0.5 text-[9px] font-bold uppercase text-danger">EN VIVO</span>}
                </p>
              )}
            </div>
            ...
            <div className="mb-3">
              ...
              <div className="mt-1 flex items-center gap-2">
                {bet.avatar ? (
                  ...
                  <span className="grid h-7 w-7 place-items-center rounded-full border border-gold/40 bg-gold/10 text-xs font-bold text-gold">
                    {bet.user[0]}
                  </span>
                )}
                <div>
                  <p className="text-sm font-semibold text-strong">{bet.user}</p>
                  <p className="text-[11px] text-subtle">apostó {timeAgo(bet.created_at)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-card-border pt-3">
              <p className="text-xs text-muted">
                <b className="text-foreground">{bet.backers}</b> {bet.backers === 1 ? 'persona apostó' : 'personas apostaron'} a este mismo pick.
              </p>
            </div>
          </div>
        </div>
